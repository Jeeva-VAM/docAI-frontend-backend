...
class ChunkedUploadService {
  ...
  async uploadFile(
    ...
    console.log(`📦 Starting chunked upload for ${file.name} (${this.formatBytes(file.size)}) in ${total_chunks} chunks of ${this.formatBytes(chunkSize)} each`);
    ...
    try {
      ...
      console.log(`✅ Chunked upload initialized:`, initResponse);
    ...
    // Step 3: Complete the upload (pass file + total_chunks)
    ...
    console.log(`✅ Chunked upload completed:`, completeResponse);
      ...
      console.error(`❌ Chunked upload failed for ${file.name}:`, errorMsg);
  ...
  private async initializeChunkedUpload(
    ...
    console.log(`� Initializing chunked upload:`, { url, requestBody });
  ...
  private async uploadSingleChunk(
    ...
    console.log(`📤 Uploading chunk ${chunk.chunkIndex + 1} (${this.formatBytes(chunk.chunkSize)})`);
    ...
    console.log(`✅ Chunk ${chunk.chunkIndex + 1} uploaded successfully`);
  }
 ...
 private async completeChunkedUpload(
  ...
  console.log(`🔄 Completing chunked upload:`, { url, requestBody });
